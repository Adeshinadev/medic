...
def patientReport(request):
    if request.method == "POST":
        time_of_the_day = request.POST['time_of_the_day']
        response = request.POST['response']
        patient_fed = Patients.objects.get(admin=request.user.id)
        save_usage = Daily_usage(patient_id=patient_fed, time_of_the_day=time_of_the_day, status=response)
        save_usage.save()
        messages.success(request, "Response recoded!")
        return redirect("patient_report")
    else:
        patient_obj = Patients.objects.get(admin=request.user.id)

        patient_dispen = patient_obj.dispense_set.all().count()
        submitted_reports = Daily_usage.objects.filter(date_time__date=today)
        submitted_report_morning = Daily_usage.objects.filter(date_time__date=today, time_of_the_day="Morning")
        submitted_report_afternoon = Daily_usage.objects.filter(date_time__date=today, time_of_the_day="Afternoon")
        submitted_report_evening = Daily_usage.objects.filter(date_time__date=today, time_of_the_day="Evening")

        context = {
            "total_disp": patient_dispen,
            "submitted_reports": submitted_reports,
            "submitted_report_morning": submitted_report_morning,
            "submitted_report_afternoon": submitted_report_afternoon,
            "submitted_report_evening": submitted_report_evening,

        }

        print(submitted_reports, "hjheu")

        return render(request, 'patient_templates/patient_report.html', context)
